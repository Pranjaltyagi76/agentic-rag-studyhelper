...
| 2026-07-14 | Embedding **runtime** = **fastembed (ONNX)**, not sentence-transformers (PyTorch). Model unchanged (all-MiniLM-L6-v2, 384-dim). | ~15 MB + a one-time ~83 MB ONNX model vs ~2.5 GB torch; far lighter for free HF Spaces, and it sidestepped a dependency conflict in the original frozen requirements. `onnxruntime` was already a dep. |
| 2026-07-14 | Replaced the bloated/broken `pip freeze` `requirements.txt` with a **curated list** + generated `requirements.lock.txt`. | Original was missing imported packages and had conflicting pins; venv built cleanly on Python 3.13 from the curated list. |
| 2026-07-15 | Pre-deploy hardening: configurable CORS, optional `APP_API_KEY` gate, production refuses SQLite, sanitized 500s. | Close the "wide-open API" gaps before Phase 9. Full per-user auth remains future work (v1 relies on unguessable session tokens + the optional API key). |
| 2026-07-15 | **Deploy host changed: HF Spaces → Render free web service.** Neon (Postgres + pgvector) unchanged. | HF locked the **Docker SDK behind a paid plan** and removed free CPU Basic (no announcement; community backlash) — Static-only is useless for a Python API. Render's free web service is still card-free with Docker support (750 hrs/mo, 15-min idle spin-down). Render's original blockers (30-day expiring Postgres, paid disks) no longer apply because state **and** vectors live in Neon/pgvector. |
