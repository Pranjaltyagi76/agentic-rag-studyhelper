...
| A16 | 🟠 | Structured-output LLM calls (planner, retrieval-plan, grader, groundedness, quiz, eval) have **no retry/fallback**. Groq intermittently returns `tool_use_failed` on inputs with apostrophes/quotes (e.g. "Newton's laws") → the request 500s. Wrap `with_structured_output(...)` calls in a retry (and/or a plain-JSON fallback). Matches the strategy checklist "structured-output calls handle validation failure". | `app/agent/*` | recommended next |
