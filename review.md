...
| A9 | ✅ 🟡 | ~~CORS `allow_origins=["*"]` with credentials.~~ **RESOLVED:** origins from `CORS_ALLOW_ORIGINS` (credentials only with explicit origins); + optional `APP_API_KEY` gate on all endpoints; + production refuses SQLite; + 500s log server-side and hide details in prod. Verified (gate 401/200, prod-sqlite blocked). | `app/main.py`, `app/config.py` | ✅ |
| A17 | 🔵 | On Groq **429 rate limit** the teacher's plain `model.invoke` (lesson gen) isn't wrapped, so it surfaces as the 500 error envelope (stream: an error event) rather than a friendly "rate limit, try later" 429. Acceptable degradation; nicer message would help on the free tier. | `app/agent/teacher.py` | future |
